// src/pages/dashboards/retailer/RetailerInventory.jsx
...
function InventoryCard({ product, onEdit, onDelete, theme }) {
  return (
    <div
      style={{
        width: "100%",
...
export default function RetailerInventory() {
  ...
  // Get theme from DashboardLayout context (emerald for retailer)
  const theme = {
  ...
  const save = async (obj) => {
    ...
    if (!obj.product_price || Number(obj.product_price) <= 0) return warning("Valid price is required");
    ...
    try {
      ...
      const res = await fetch(
        "http://localhost:5000/api/retailer/inventory",
        {
          method: "POST",
          headers: {
            "Authorization": `Bearer ${token}`,
      ...
      success(`Product ${isEditMode ? "updated" : "added"} successfully!`);

      setItems((prev) => {
        const exists = prev.some((x) => x.product_id === saved.product_id);
        if (exists) return prev.map((x) => (x.product_id === saved.product_id ? saved : x));
        return [saved, ...prev];
      });
  ...
  const del = async (product_id) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/retailer/del/del", {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const uniqueCategories = ["All", ...new Set(items.map(item => item.product_category).filter(Boolean))];
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <div style={{ color: "rgb(148, 163, 184)", fontSize: 18, marginBottom: 8 }}>
            {items.length === 0 ? "No products yet" : "No products found"}
          </div>
          <div style={{ color: "rgb(148, 163, 184)", fontSize: 14 }}>
            {items.length === 0 ? "Click 'Add Product' to get started" : "Try adjusting your search or filter"}
          </div>
        </div>
      ) : (
        <div style={{
